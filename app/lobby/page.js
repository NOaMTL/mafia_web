  ...
  return (
    <main className="page meta-page lobby-home lobby-browser-page">
      ...
      <PageHeading
        ...
        subtitle="Rejoignez immédiatement un salon public, ou créez une table privée réservée à vos invités."
      />

      <div className="lobby-choice-grid lobby-entry-grid">
          ...
          <form className="lobby-create-form" onSubmit={createLobby}>
            ...
            <div className="lobby-visibility-picker" role="group" aria-label="Visibilité du salon">
              ...
              <button type="button" className={!isPublic ? 'active private' : ''} onClick={() => setIsPublic(false)}>
                <span>⌁</span><b>PRIVÉ</b><small>Code obligatoire</small>
              </button>
            </div>
        ...
        <section className="choice-card lobby-private-join-card">
          <div className="choice-number">02</div>
          <div className="choice-icon">⌁</div>
          <h2>REJOINDRE PAR CODE</h2>
          <p>Les tables privées ne sont jamais listées. Saisissez le code transmis par l’hôte.</p>
          <form onSubmit={joinLobby} className="join-form">
            ...
            />
            <button disabled={Boolean(busy) || code.length !== 6} type="submit">REJOINDRE</button>
          </form>
          <div className="private-lobby-note">
            <span>◈</span>
            <div><strong>ACCÈS CONFIDENTIEL</strong><p>Un lien interne ne suffit pas : le code reste obligatoire pour entrer.</p></div>
          </div>
        </section>
      </div>
