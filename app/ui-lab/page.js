...
function LabMayorRevealControl() {
  ...
  return (
    <section className={`mayor-reveal-control ${revealed ? 'is-active' : ''} ${confirming ? 'is-confirming' : ''}`} aria-label="Pouvoir du Maire">
      ...
      <div className="mayor-reveal-copy"><small>POUVOIR UNIQUE · MAIRE</small><strong>{revealed ? 'AUTORITÉ PUBLIQUE ACTIVE' : confirming ? 'RÉVÉLATION IRRÉVERSIBLE' : 'RÉVÉLER TON IDENTITÉ'}</strong><span>{revealed ? 'Tes bulletins valent désormais trois voix.' : confirming ? 'Toute la ville connaîtra ton rôle.' : 'Deviens public et renforce chacun de tes votes.'}</span></div>
      {revealed ? <div className="mayor-reveal-active"><i /> ACTIF <b>VOTE ×3</b></div>
        : confirming ? <div className="mayor-reveal-confirm-actions"><button type="button" className="mayor-confirm-button" onClick={() => setState('revealed')}>CONFIRMER <b>×3</b></button><button type="button" className="mayor-cancel-button" onClick={() => setState('ready')}>ANNULER</button></div>
          : <button type="button" className="mayor-reveal-button" onClick={() => setState('confirming')}><span>SE RÉVÉLER</span><b>VOTE ×3</b></button>}
    </section>
  );
}
...
function LabRoleReveal({ role }) {
  return <main className="role-reveal-screen"><div className="reveal-vignette" /><section className={`role-reveal-stage ${role.team === 'MAFIA' ? 'mafia' : 'village'}`}><div className="reveal-kicker"><span /> VOTRE IDENTITÉ POUR CETTE NUIT <span /></div><div className="role-card-wrap"><div className="role-card-aura" /><div className="role-reveal-card"><img className="role-reveal-art" src={role.image} alt="" /><div className="role-reveal-scrim" /><div className="role-reveal-overlay"><div className="role-reveal-topline"><span><i />{role.team === 'MAFIA' ? 'MAFIA' : 'VILLE'}</span><b>LG-03</b></div><div className="role-reveal-copy"><div className="role-reveal-kicker">DOSSIER DE RÔLE · IDENTITÉ CONFIDENTIELLE</div><h1 className="role-name">{role.name.toUpperCase()}</h1><div className="role-team-name">{role.team === 'MAFIA' ? '◆ FAMILLE MAFIA' : '✦ ALLIANCE TOWN'}</div><p>{role.description}</p><div className="role-reveal-power"><span>{role.nightAction ? 'MISSION NOCTURNE' : 'RÔLE PASSIF'}</span><strong>{role.nightAction ?? role.tip}</strong></div><div className="role-secret"><span>◈</span> NE RÉVÉLEZ VOTRE IDENTITÉ À PERSONNE</div></div></div></div></div><div className="role-reveal-countdown"><div><span>LA PARTIE COMMENCE</span><strong>8s</strong></div><div className="reveal-progress"><span style={{ width: '78%' }} /></div></div></section></main>;
}
...
function LabOverlay({ type, role, onClose }) {
  ...
  if (type === 'lobbyBrowser') return <LabPublicLobbyBrowser />;
  ...
  if (type === 'lobbyBot') return <div className="ui-lab-lobby-layer"><section><small>SALON · CONTRÔLE HÔTE</small><h2>KICK & BANNISSEMENT</h2><article className="lobby-player-row is-ready"><span className="lobby-player-index">03</span><span className="lobby-player-avatar">M</span><span className="lobby-player-identity"><strong>Marcel</strong><small>INVITÉ</small></span><span className="lobby-player-badges"><b>JOUEUR</b></span><span className="lobby-player-actions"><span className="lobby-ready-state ready"><i />PRÊT</span><div className="lobby-moderation-actions"><button className="lobby-kick-btn is-player"><span>−</span><b>KICK</b></button><button className="lobby-ban-btn"><span>⊘</span><b>BAN</b></button></div></span></article><article className="lobby-player-row is-ready"><span className="lobby-player-index">04</span><span className="lobby-player-avatar">🤖</span><span className="lobby-player-identity"><strong>Lucien</strong><small>JOUEUR AUTOMATIQUE</small></span><span className="lobby-player-badges"><b>BOT</b></span><span className="lobby-player-actions"><span className="lobby-ready-state ready"><i />PRÊT</span><div className="lobby-moderation-actions"><button className="lobby-kick-btn is-bot"><span>−</span><b>RETIRER</b></button></div></span></article></section></div>;
  return null;
}

function LabPublicLobbyBrowser() {
  const lobbies = [
    { id: 'lab-1', name: 'Les Ombres de Paris', hostUsername: 'Violette', playerCount: 7, humanCount: 6, botCount: 1, maxPlayers: 10 },
    { id: 'lab-2', name: 'Partie débutants', hostUsername: 'Lucien', playerCount: 4, humanCount: 4, botCount: 0, maxPlayers: 8 },
  ];
  return <div className="ui-lab-public-browser-layer"><section className="public-lobby-browser"><header className="public-lobby-browser-heading"><div><small>TABLES OUVERTES · CONNEXION DIRECTE</small><h2>SALONS PUBLICS</h2><p>Choisissez une table : votre place est réservée dès que vous la rejoignez.</p></div><div className="public-browser-status"><span><i /> ACTUALISATION AUTO · 5S</span><button>↻ ACTUALISER</button></div></header><div className="public-lobby-grid">{lobbies.map((lobby, index) => <article className="public-lobby-card" key={lobby.id}><div className="public-lobby-index">{String(index + 1).padStart(2, '0')}</div><div className="public-lobby-card-topline"><span><i /> SALON PUBLIC</span><time>OUVERT IL Y A {index + 2} MIN</time></div><h3>{lobby.name}</h3><div className="public-lobby-host"><span>{lobby.hostUsername[0]}</span><div><small>HÔTE DE LA TABLE</small><strong>{lobby.hostUsername}</strong></div></div><div className="public-lobby-occupancy"><div><span>JOUEURS</span><strong>{lobby.playerCount}<small>/{lobby.maxPlayers}</small></strong></div><div><span>HUMAINS</span><strong>{lobby.humanCount}</strong></div><div><span>BOTS</span><strong>{lobby.botCount}</strong></div></div><div className="public-lobby-meter"><span style={{ width: `${(lobby.playerCount / lobby.maxPlayers) * 100}%` }} /></div><button><span>REJOINDRE LA TABLE</span><b>→</b></button></article>)}</div><footer><span>2 SALONS DISPONIBLES</span><small>DONNÉES DE PRÉVISUALISATION</small></footer></section></div>;
}
